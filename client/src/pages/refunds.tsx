export default function Refunds() {
  return (
    <div className="flex-1 py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-4xl font-bold mb-8 font-serif italic">Refund Policy</h1>
        
        <div className="prose prose-lg dark:prose-invert max-w-none">
          <p className="text-muted-foreground mb-8">
            Last updated: February 2026
          </p>

          <h2>30-Day Satisfaction Guarantee</h2>
          <p>
            We want you to love your art. If you're not completely satisfied with your purchase, 
            we'll make it right.
          </p>

          <h2>Digital Downloads</h2>
          <p>
            Due to the nature of digital products, we generally do not offer refunds for digital 
            downloads. However, if you're unhappy with your transformation, contact us and we'll 
            work with you to create a new version at no extra cost.
          </p>

          <h2>Printed Products</h2>
          <p>
            If your printed product arrives damaged or doesn't meet your expectations, please 
            contact us within 30 days of delivery. We'll either:
          </p>
          <ul>
            <li>Send you a replacement at no additional cost</li>
            <li>Provide a full refund</li>
            <li>Work with you on a new design</li>
          </ul>

          <h2>How to Request a Refund</h2>
          <p>
            To request a refund or report an issue:
          </p>
          <ol>
            <li>Contact our support team at support@arttransform.com</li>
            <li>Include your order number and a description of the issue</li>
            <li>For damaged products, please include photos</li>
          </ol>
          <p>
            We aim to respond to all refund requests within 2 business days.
          </p>

          <h2>Exceptions</h2>
          <p>
            Refunds may not be available for:
          </p>
          <ul>
            <li>Orders placed more than 30 days ago</li>
            <li>Products damaged due to customer handling</li>
            <li>Changes of mind after digital download</li>
          </ul>

          <h2>Contact Us</h2>
          <p>
            Questions about refunds? Email us at support@arttransform.com or visit our 
            Support page.
          </p>
        </div>
      </div>
    </div>
  );
}
